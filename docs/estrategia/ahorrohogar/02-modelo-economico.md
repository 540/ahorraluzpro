...
## Decisiones / hipótesis que vienen de la visión

> Estas decisiones se tomaron al definir la visión y condicionan el modelo. Confirmarlas o ajustarlas al desarrollar `/init-producto modelo`.

- **Modo de operación = notificar + asistir** (NO ejecutar el cambio por el usuario). Implicación: la plataforma no necesita representación legal del usuario ni integración con comercializadoras, lo cual mantiene el stack 100% frontend.
- **Frecuencia de uso y vigencia de los datos pendiente de decidir:** ¿asumimos consumo constante en el tiempo o pedimos al usuario que reescanee su factura cada X meses (3, 6, 12)? Esto condiciona la calidad del aviso de cambio y la frecuencia de uso de la plataforma. Es decisión clave del modelo y debe resolverse aquí.
